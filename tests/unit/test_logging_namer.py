...
def test_namer_pads_and_places_before_extension(tmp_path) -> None:
...
def test_namer_rotation_folder_creates_directory(tmp_path) -> None:
...
def test_namer_non_numeric_suffix_returns_same(tmp_path) -> None:
    handler = SimpleNamespace(backupCount=5)
