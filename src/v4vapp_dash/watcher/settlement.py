...
class Decision:
    ...
    duffs_received: int
    sats_credited: int | None
    late_payment: bool
    txids: list[dict[str, Any]]
    stuck: bool = False


def underpay_tolerance(duffs_quoted: int, bps: int, floor_duffs: int) -> int:
    return max(floor_duffs, duffs_quoted * bps // 10_000)
...
def apply_settlement(
    *,
    ...
    qual_sum = sum(out.duffs for out in qualifying)
    total = sum(out.duffs for out in outputs)
    tolerance = underpay_tolerance(
        duffs_quoted,
        int(policy.get("underpay_tolerance_bps") or 0),
        int(policy.get("underpay_tolerance_duffs") or 0),
    )
    ...
    if qual_sum >= duffs_quoted:
        new_state = (
            DashInvoiceState.OVERPAID if qual_sum > duffs_quoted else DashInvoiceState.SETTLED
        )
        return Decision(
            state=new_state,
            ...
            settled_at=now,
            duffs_received=total,
            sats_credited=sats_requested,
            late_payment=late_payment,
            txids=txids,
        )

    if qual_sum >= duffs_quoted - tolerance:
        return Decision(
