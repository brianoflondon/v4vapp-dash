...
async def bootstrap_watch_wallet(
    ...
    *,
    ...
    try:
        ...
        logger.warning(
            "dashd wallet RPC is disabled (-disablewallet=1); skip watch-wallet bootstrap"
        )
    ...
    if dashd.wallet_name not in wallets:
        ...
        created = True
        logger.info("created watch-only descriptor wallet %s", dashd.wallet_name)
    ...
    if need_import:
        ...
        imported = True
        logger.info("imported watch descriptors range [0, %s]", range_end)
