...
from v4vapp_dash.limits.hive_config import RateWindow, fetch_rate_windows
...
def _sats_of(doc: dict[str, Any]) -> int:
    credited = doc.get("sats_credited")
    if credited is not None:
        return int(credited)
    return int(doc.get("sats_requested") or 0)
...
def _period_result(
    *,
    ...
    total = used + extra
    percent = int((total / window.sats) * 100) if window.sats else 0
    ...
    return {
        ...
        "limit_sats": window.sats,
        "sats": total,
        "sats_paid": used,
        "sats_requested": extra,
        "limit_ok": ok,
    ...
    *,
    ...
    for window in windows:
        ...
        in_window = [
            doc
            for doc in paid
            if doc.get("settled_at") is not None and doc["settled_at"] >= since
        ]
        used = sum(_sats_of(doc) for doc in in_window)
        result = _period_result(window=window, used=used, extra=extra_sats, cust_id=cust_id)
